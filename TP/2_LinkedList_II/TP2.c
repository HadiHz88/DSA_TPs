...
// Ex5: Function to add two numbers represented by linked lists
ListNode *addNumbers(ListNode *num1, ListNode *num2)
{
    // Handle edge cases where one of the numbers is NULL
    if (num1 == NULL)
    {
        return num2;
    }
    ...
    ListNode *result = NULL; // Head of the result linked list
    ListNode *tail = NULL;   // Tail pointer to keep track of the last node
    int carry = 0;           // Carry digit for addition

    // Continue looping until both lists are processed and there's no carry left
    while (num1 != NULL || num2 != NULL || carry != 0)
    {
        int sum = carry; // Initialize sum with the carry from previous addition

        // Add current digit from first number if available
        if (num1 != NULL)
        {
            sum += num1->data;
            num1 = num1->next;
        ...
        // Add current digit from second number if available
        if (num2 != NULL)
        {
            sum += num2->data;
            num2 = num2->next;
        ...
        carry = sum / 10;     // Calculate carry for next iteration (1 if sum >= 10, otherwise 0)
        int digit = sum % 10; // Calculate the digit to add to the result (remainder after division by 10)

        // Create a new node for the current digit
        ListNode *newNode = createListNode(digit);

        // If result is empty, initialize both result and tail
        if (result == NULL)
        {
            result = newNode;
            tail = newNode;
